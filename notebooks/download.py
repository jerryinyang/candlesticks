...
async def upload_file_to_s3(file_path, bucket, s3_key):
    ...
    async with session.create_client('s3') as s3_client:
        try:
            with open(file_path, 'rb') as file_data:
                await s3_client.put_object(Bucket=bucket, Key=s3_key, Body=file_data)
            return True
        except Exception as e:
            print(f"Error uploading file to S3: {e}")
            return False

async def fetch_binance_data(symbol, timeframe, _from: datetime.datetime, to: datetime.datetime, batch_size=1000):
    ...
    try:
        ...
        try:
            df = pd.read_csv(f'.cache/ohlcv3/{symbol}/{symbol}_{timeframe}.csv')
        ...
        while True:
            ...
            ohlcv = await binance_client.public_get_klines({
                'symbol': symbol,
                'interval': timeframe,
                'limit': batch_size,
            ...
            symbol_folder = f'.cache/ohlcv3/{symbol}'
            ...
            await upload_file_to_s3(batch_file_path, s3_bucket_name, f'ohlcv3/{symbol}/{os.path.basename(batch_file_path)}')
...
async def fetch_many_binance_data(symbols, timeframe, _from, to, batch_size):
    try:
        async with Pool() as pool:
            args = [(symbol, timeframe, _from, to, batch_size) for symbol in symbols]
            result = await pool.starmap(fetch_binance_data, args)
        total_df = pd.concat(result, ignore_index=True)
        return total_df
    except Exception as e:
        print(f"Error in fetch_many_binance_data: {e}")
        return pd.DataFrame()

async def get_bybit_symbols():
    bybit_client = ccxt.bybit()
    try:
        markets = await bybit_client.load_markets()
...
async def main(symbols, timeframe, from_date, to_date, batch_size=1000):
    """
    ...
    try:
        ...
        total_df = await fetch_many_binance_data(symbols, timeframe, _from, to, batch_size)
...
async def init_and_run():
    try:
        ...
        all_pairs = [market.replace("/", '') for market in binance.symbols if market.endswith('/USDT')]
        ...
        await main(symbols, timeframe, from_date, to_date, batch_size)
